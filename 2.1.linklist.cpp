...
typedef struct LNode {
	...
	struct LNode *next; //结点的指针域
                    ...
                    //结构体类型变量LNode，结构体类型指针 LinkList 
				   
bool InitList_L(LinkList &L)//构造一个空的单链表L，&L：表示实参的引用 
{                     //分配成功会返回一个指针 
...
void CreateList_H(LinkList &L)//前插法创建单链表,&L:表示实参的引用 
{
	//输入n个元素的值，建立带头结点的单链表L
	int n;
	LinkList s; //定义一个指针变量
	L=new LNode; //带有头节点的，空链表 
...
bool LocateElem_L(LinkList L, int e) //按值查找
{
	...
	LinkList p;
	p=L->next;//p指向第一个结点
	while (p && p->data!=e)//顺链域向后扫描，直到p为空或p所指结点的数据域等于e,&&(与=且) 
	{	
	    p=p->next; //p指向下一个结点
	} 
	if(!p)
